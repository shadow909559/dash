...
## #143 - Repo-wide phantom-call gate: calling functions that don't exist is now a CI failure (2026-09-22)

**Why.** #141/#142 shared one root cause: code called functions that never existed on their target module (`TaskQueue.enqueue`, `WorkflowEngine.run_workflow`, `voice_system.service.transcribe_audio`, `rag.embeddings.embed_texts`, `TaskPriority.MEDIUM`). Every gate was blind — syntax-valid, importable, and broad excepts converted the AttributeError into fabricated success. Static analysis is the only place this class can be caught before runtime.

**The gate.** `scripts/check_phantom_calls.py` — AST-based, cross-module: resolves `import X` / `from X import Y` bindings against the real module surfaces inside `dash_backend` + `scripts` and flags calls to attributes that provably don't exist (both bare-name from-imported calls and `binding.attr()` calls). Deliberately near-zero false positives: instance calls, star imports, monkeypatching, and out-of-package modules are out of scope by design; `__all__` overrides a package's visible surface; and package surface includes **submodules** (`from pkg import service` is legal Python even when `__init__.py` never re-exports it — the runtime binds imported submodules as attributes; the first checker draft violated this and produced 117 false positives, caught empirically before shipping).

**Real phantoms it caught on the first run (beyond the earlier fixes):** `plugins/api.py` imported nonexistent `add_memory` (real: `save_memory`) — plugin `memory_save` ALWAYS failed into its warning-and-None fallback; `android_agent` imported nonexistent `get_phone_status` (real surface: `PhoneSkill.handle`) — its "real path" never worked. Both fixed. `diagnostics._websocket_health` imported a submodule that doesn't exist (`routes.websocket.connection_manager`), so WS health reported "unknown" forever — now reads the real tracker via `system_ws.active_connections_count()` (verified end-to-end: 2 planted connections → count 2).

**Pins + CI.** `tests/test_phantom_calls.py` (8 pins): clean real tree (no drift), synthetic mini-trees prove each phantom shape is flagged (missing attr on import binding, missing from-import symbol) and each legal shape is not (submodule imports, `__all__`, star, instance calls, out-of-package). The checker runs as a hard CI step in the lint job. Meta-lesson re-learned: the checker's own test file tripped the #138 undeclared-import gate via a top-level `from scripts...` import — test now imports lazily, function-scoped.

## #144 - Frontend↔backend route parity: every desktop API call now provably hits a real route (2026-09-22)

**Why.** A repo-wide cross-check of every API call target extracted from the desktop app source against the live OpenAPI route table found 21+ real gaps: pages calling endpoints that never existed, raw `fetch()` calls missing the device-token auth header (guaranteed 401), double `/api/v1` prefixes, and response-shape mismatches (backend wraps in `{"items": ...}` / `{"contacts": ...}`, pages expected bare arrays). Every one of these was a button that silently did nothing or crashed at runtime — invisible to every existing gate.

**Backend — routes added where a real service already existed (no new fakes):** phase4: sprint create, sprint-items CRUD, clipboard pin/delete, screenshots capture/delete, debug-log clear, updates check, time-entries manual entry, sandboxed terminal exec (same guards as the code-execution tool: allowed roots, timeout, output caps); phase3: archives create/restore/delete; phase2: contacts delete, meetings CRUD. Services extended honestly: `MeetingNotesService.create` accepts the fields the page sends, `ArchiveService.delete`, `ContactService` delete returns real found/not-found, reminder ids collision-proof after delete, action-items accept description/tags, `FileOperationResponse`-shaped files write.

**Frontend — 18 pages fixed:** converted raw `fetch` to `authFetch` (ContactManager, Phone, CodeEditor, SessionReplay, DebugConsole, UpdateChecker, TimeTracking, Terminal, ReadingList, BackupRestore, ScreenshotCapture, FileBrowser…), corrected prefixes (RAG lives at `/rag/*`, not `/documents`; file-browser root token is `home`, not `/`; recordings at `/desktop/recordings`; `/phone/status`; `/updates/check`), and normalized wrapped-vs-bare shapes at the call site (kept backend GET shapes intact so existing `dash_web` consumers are unaffected).

**The gate.** `scripts/_fe_route_check.py` extracts every call target from `apps/desktop/src` (fetch/authFetch first args, API_BASE-relative and template literals with `${...}` mapped to FastAPI `{param}` syntax) and matches against the route table. Semantics learned empirically: a frontend dynamic `*` must match any backend segment including literals (approve/pause/resume dispatch), block comments are stripped (no JSDoc false positives), external URLs and non-API literals are skipped, `${RAG}`-style base constants expand to their real prefixes, and bare `fetch('/…')` is extracted (the historically broken pattern). Current tree: **zero unmatched call targets**.
