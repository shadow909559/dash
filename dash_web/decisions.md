...
### 28. CloudFront HTTPS distribution for website

**Decision:** Use CloudFront in front of S3 website endpoint to enable HTTPS, HTTP/2, HTTP/3, gzip compression, and global CDN caching.

**Why:**
- S3 website endpoints only support HTTP — no HTTPS
- CloudFront provides free SSL via AWS Certificate Manager
- HTTP/2 and HTTP/3 for faster page loads
- Global edge caching reduces latency worldwide
- 403/404 custom error responses route back to index.html for SPA routing
- Assets cached for 30 days, HTML cached for 1 day
- Account verification required for new AWS accounts before CloudFront creation

**Fallback:** Direct S3 HTTPS URL (`https://bucket.s3.region.amazonaws.com/index.html`) works without CloudFront for specific file access.

**Deploy script:** `dash_web/deploy.sh` handles build → S3 sync → CloudFront creation/invalidation.

### 29. Android APK distribution — GitHub Release asset + S3 backup

**Decision:** Host the Android APK as a GitHub Release asset (canonical) with an S3 copy as a CDN backup.

**Why:**
- GitHub Releases is the single source of truth the website already links to
- S3 copy provides a fast CDN fallback and survives release-token permission issues
- APK built with release signing config (`my-upload-key.jks`) and Java 21 (Android Studio JBR — Java 26 breaks AGP jlink)
- Fixed pre-existing mobile build break: `AgentModeScreen.kt` used `GlassCard` without importing it
- Release upload requires the `gho_` OAuth token from Windows credential manager (fine-grained PAT lacks upload permission)
