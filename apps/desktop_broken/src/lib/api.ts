...
export const memory = {
  getAll: (page = 1, perPage = 20) => {
    ...
    return request<{ items: Array<{ id: string; content: string; type: string; created_at: string }>; total: number }>(
  ...
  search: (query: string) =>
    request<{ items: Array<{ id: string; content: string; type: string; score: number }> }>(
