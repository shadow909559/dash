...
class TestPlannerExecutiveIntegration:
    ...
    def _no_live_llm(self, monkeypatch: pytest.MonkeyPatch):
        """Planner decomposition must be hermetic (no live Ollama calls)."""
        import dash_backend.executive.planner as planner_mod

        async def fake_collect(_messages):
            return json.dumps(
                [
                    {"name": "Step 1", "description": "Do step 1", "depends_on": []},
                    {"name": "Step 2", "description": "Do step 2", "depends_on": ["Step 1"]},
                ]
            )

        monkeypatch.setattr(planner_mod, "collect_streamed_response", fake_collect)
