...
class TestAuthorization:
    """Memory and conversation ownership verification."""

    async def _register_and_login(self, client, email, username):
    ...
    async def test_memory_ownership_enforced(self, security_client):
        # Register two users
        user1 = await self._register_and_login(security_client, "alice@test.com", "alice")
        user2 = await self._register_and_login(security_client, "bob@test.com", "bob")

        # Alice creates a memory
        alice_token = user1["access_token"]
        mem_resp = await security_client.post(
            "/api/v1/memory",
            json={"content": "Alice's secret", "category": "general"},
            headers={"Authorization": f"Bearer {alice_token}"},
        )
        assert mem_resp.status_code == 201
        memory_id = mem_resp.json()["id"]

        # Bob tries to read Alice's memory
        bob_token = user2["access_token"]
    ...
    async def test_conversation_ownership_enforced(self, security_client):
        user1 = await self._register_and_login(security_client, "carol@test.com", "carol")
        user2 = await self._register_and_login(security_client, "dave@test.com", "dave")

        # Carol creates a conversation
        carol_token = user1["access_token"]
        conv_resp = await security_client.post(
            "/api/v1/conversations",
            json={"title": "Carol's conversation"},
            headers={"Authorization": f"Bearer {carol_token}"},
        )
        assert conv_resp.status_code == 201
        conv_id = conv_resp.json()["id"]

        # Dave tries to access Carol's conversation
        dave_token = user2["access_token"]
        ...
        # Dave tries to get messages
        msg_resp = await security_client.get(
            f"/api/v1/conversations/{conv_id}/messages",
            headers={"Authorization": f"Bearer {dave_token}"},
        )
        assert msg_resp.status_code == 403

        # Dave tries to update
        update_resp = await security_client.patch(
            f"/api/v1/conversations/{conv_id}",
            json={"title": "Dave's hijack"},
            headers={"Authorization": f"Bearer {dave_token}"},
        )
        assert update_resp.status_code == 403

        # Dave tries to delete
        delete_resp = await security_client.delete(
            f"/api/v1/conversations/{conv_id}",
            headers={"Authorization": f"Bearer {dave_token}"},
        )
