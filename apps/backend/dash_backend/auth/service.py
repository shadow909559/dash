...
async def revoke_all_refresh_tokens(session: AsyncSession, user_id) -> int:
...
async def issue_token_response(
    ...
    *,
    ...
    When a refresh token is created, a Session record is also created so
    the session lifecycle can be tracked and individual sessions revoked.
    """
    ...
    # Create a session record for lifecycle tracking
    try:
        from dash_backend.auth.session_service import create_session
        await create_session(
            session, user.id, refresh_token,
            ip_address=ip_address, user_agent=user_agent,
        )
    except Exception:
        # Session creation is best-effort; token issuance must not fail
        pass
