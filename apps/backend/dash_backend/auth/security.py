...
def _get_secret_key() -> bytes:
    ...
    if secret_key is None:
        # Only when the secret is unset (absent env var) may development/test
        # environments fall back to a fixed dev key so tests and local
        # development do not require an env var. Do NOT use this in production.
        if settings.env in ("development", "test"):
            dev_key = "dash_dev_jwt_secret_key"
            return dev_key.encode("utf-8")
        raise AuthConfigurationError("DASH_JWT_SECRET_KEY must be configured")
    if not secret_key:
        # An explicitly-provided empty secret is invalid in every environment.
        raise AuthConfigurationError("DASH_JWT_SECRET_KEY must be configured")
