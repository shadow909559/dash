...
from dash_backend.context.engine import build_context_block
...
async def handle_chat_send(
    ...
    for step in range(MAX_TOOL_STEPS + 1):
        ...
        # voice mode to keep the interaction snappy.
        if not getattr(msg, "voice_mode", False):
            try:
                env_block = await build_context_block(session=session, user_id=user_id)
                if env_block:
                    system_prompt = (
                        f"{system_prompt}\n\n"
                        "[ENVIRONMENT CONTEXT - live snapshot of the user's machine. "
                        "Use it to ground answers; never repeat it verbatim]\n"
                        f"{env_block}"
                    )
            except Exception:
                logger.exception("Failed to attach environment context")
