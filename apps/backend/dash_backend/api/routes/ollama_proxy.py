...
@router.post("/chat")
async def ollama_chat(request: Request, user: User = Depends(get_current_user)):
...
async def ollama_chat_stream(request: Request, user: User = Depends(get_current_user)):
