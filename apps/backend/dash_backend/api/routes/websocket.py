...
async def websocket_endpoint(websocket: WebSocket) -> None:
    ...
    try:
        while True:
            ...
            # Orchestrator: multi-agent chain for complex tasks
            elif msg_type == "orchestrator.run":
                task_desc = raw.get("task", "")
                run_id = raw.get("run_id", str(uuid.uuid4())[:12])
                if task_desc:
                    async def run_orchestrator(desc: str, rid: str):
                        from dash_backend.autonomous.orchestrator import get_orchestrator
                        orch = get_orchestrator()
                        async for event in orch.run(desc):
                            await send_json({"type": f"orchestrator.{event.type}", "run_id": rid, **event.data})
                    task = asyncio.create_task(run_orchestrator(task_desc, run_id))
                    chat_tasks.add(task)
                    task.add_done_callback(chat_tasks.discard)
                continue
