...
class MediaService(Singleton):
    ...
    async def get_brightness(self) -> dict[str, Any]:
        ...
        if not IS_WINDOWS:
            return {"brightness": 0, "summary": "Brightness not supported on this platform"}
        try:
            import wmi
            c = wmi.WMI()
            brightness = 0
            ...
            return {"brightness": 0, "summary": "Brightness control requires wmi module"}
        except Exception as exc:
            logger.exception("get_brightness failed")
            return {"brightness": 0, "summary": f"Failed: {exc}"}

    async def set_brightness(self, level: int) -> dict[str, Any]:
        """Set screen brightness (0-100)."""
        if not IS_WINDOWS:
            return {"summary": "Brightness not supported on this platform"}
        try:
            import wmi
            c = wmi.WMI()
            ...
            return {"summary": "Brightness control requires wmi module"}
        except Exception as exc:
            logger.exception("set_brightness failed")
            raise RuntimeError(f"Failed to set brightness: {exc}") from exc
