...
async def save_memory(
    ...
    *,
    ...
    if memory_type is not None:
        # Normalize: accept both "personal" and "Personal"
        resolved_type = type_map.get(memory_type.lower(), memory_type)
    else:
...
# ──────────────────────────────────────────────
# Typed Memory Categories (Part 9-10)
# ──────────────────────────────────────────────
...
MEMORY_TYPES = {
    ...
    "experience": "Past experiences and lessons learned",
}
...
async def save_typed_memory(
    ...
    *,
    ...
    """Save a memory with a specific typed category (Part 9-10)."""
    if memory_type not in MEMORY_TYPES:
        raise ValueError(
            f"Invalid memory type '{memory_type}'. "
            f"Valid types: {', '.join(MEMORY_TYPES)}"
        )
    return await save_memory(
        session, user_id, content,
        memory_type=memory_type, category=memory_type,
...
async def remember_decision(
    ...
    *,
    ...
    return await save_memory(
        session, user_id, content,
        memory_type="Decision", category="decision",
        importance=importance, confidence=confidence,
        source="decision_record", project_id=project_id,
        title=title, tags=["decision"],
    )


# ──────────────────────────────────────────────
# Continue Where We Left Off (Part 9-10)
# ──────────────────────────────────────────────


async def record_work_context(
    session: AsyncSession,
    user_id: str | uuid.UUID,
    *,
    project_id: str | uuid.UUID | None = None,
    ...
    """Record current work context for later resumption."""
    parts = [f"Working on: {task}"]
    if progress:
    ...
    content = ". ".join(parts) + "."
    title = f"Work context: {task[:100]}"
    return await save_memory(
        session, user_id, content,
        memory_type="Experience", category="experience",
        importance=0.6, confidence=0.9,
        source="work_context", project_id=project_id,
        title=title, tags=["work_context", "resume"],
    )
...
async def get_continue_context(
    ...
    *,
    ...
    uid = _safe_uuid(user_id)
    pid = _safe_uuid(project_id) if project_id else None
    filters = [
        Memory.user_id == uid,
        Memory.source == "work_context",
    ]
    if pid is not None:
        filters.append(Memory.project_id == pid)
    stmt = (
...
async def get_project_memories(
    ...
    *,
    ...
    """Retrieve memories associated with a specific project."""
    uid = _safe_uuid(user_id)
    pid = _safe_uuid(project_id)
    filters = [Memory.user_id == uid, Memory.project_id == pid]
    if memory_type is not None:
        filters.append(Memory.type == memory_type)
    stmt = (
        select(Memory).where(*filters)
...
async def get_memory_stats(
    ...
    """Aggregate statistics about a user's memory."""
    uid = _safe_uuid(user_id)
    ...
    type_counts_result = await session.execute(
        ...
        .where(Memory.user_id == uid).group_by(Memory.type)
    )
    type_counts = {row[0]: row[1] for row in type_counts_result.all()}
    avg_result = await session.execute(
        select(func.avg(Memory.importance), func.avg(Memory.confidence))
        .where(Memory.user_id == uid)
    )
    row = avg_result.one()
    avg_importance = float(row[0]) if row[0] else 0.0
    avg_confidence = float(row[1]) if row[1] else 0.5
    project_count = await session.scalar(
        select(func.count(func.distinct(Memory.project_id)))
        .where(Memory.user_id == uid, Memory.project_id.isnot(None))
    ) or 0
    return {
        "total": total,
        "by_type": type_counts,
