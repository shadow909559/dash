...
class Goal(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    # Planning fields (Goal Engine): 1 = urgent .. 5 = low; None deadline = anytime.
    priority: Mapped[Optional[int]] = mapped_column(nullable=True, default=3)
...
class ExecutiveTask(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    # Planning fields (Goal Engine): priority, deadline, and task dependencies
    # (list of ExecutiveTask ids that must be completed first).
    priority: Mapped[Optional[int]] = mapped_column(nullable=True, default=3)
    deadline: Mapped[Optional[datetime]] = mapped_column(nullable=True)
