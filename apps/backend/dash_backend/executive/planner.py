...
class Planner:
    ...
    async def decompose(goal_name: str, goal_description: str | None = None, max_tasks: int = 10) -> List[Dict[str, Any]]:
        ...
        # This makes the planner deterministic for tests and avoids
        # unnecessary LLM calls when the format is already correct.
        if goal_description:
            candidate = goal_description.strip()
            # If the provided description is a JSON array, treat it as the
