...
class CloseTabTool(BaseTool):
    name = "close_tab"
    description = "Close a browser tab matching a URL or title (best-effort). Focuses the matching window and sends Ctrl+W."
    ...
    async def execute(self, context: ToolContext, **kwargs: Any) -> ToolResult:
        ...
        if not match:
            return ToolResult(tool_name=self.name, status=ToolStatus.ERROR, error_message="match required")
        try:
            from dash_backend.tools.window_management_tools import _find_window

            hwnd = _find_window(str(match)) if IS_WINDOWS else None
            if hwnd is not None:
                import ctypes
            ...
            try:
                ...
                return ToolResult(tool_name=self.name, status=ToolStatus.ERROR, error_message="pyautogui required for tab close")
            return ToolResult(tool_name=self.name, status=ToolStatus.SUCCESS, summary=f"Sent Ctrl+W to window matching '{match}'")
        except Exception as exc:
...
class RefreshTabTool(BaseTool):
    name = "refresh_tab"
    description = "Refresh a browser tab matching URL or title (best-effort). Focuses the matching window and sends F5."
    parameters = [ToolParameter("match", "URL or title substring to match", required=True)]
    permission_level = PermissionLevel.RESTRICTED
    category = "browser"
    ...
    async def execute(self, context: ToolContext, **kwargs: Any) -> ToolResult:
        ...
        try:
            ...
            try:
                ...
                return ToolResult(tool_name=self.name, status=ToolStatus.ERROR, error_message="pyautogui required for tab refresh")
            return ToolResult(tool_name=self.name, status=ToolStatus.SUCCESS, summary=f"Refreshed window matching '{match}'")
        except Exception as exc:
            logger.exception("refresh_tab failed")
            return ToolResult(tool_name=self.name, status=ToolStatus.ERROR, error_message=str(exc))
