...
class ToolManager:
    ...
    def select_tool_definitions(
        ...
        *,
        ...
        """Return a RELEVANT subset of tool definitions for one LLM request.

        Sending 100+ JSON schemas exceeds local-model context windows and
        Ollama silently truncates the system prompt (memory/agent context
        disappears). We rank tools by keyword overlap between the user
        message and the tool name/description/category, then cap the list.
        """
        from dash_backend.config import get_settings

        if max_tools is None:
            max_tools = getattr(get_settings(), "max_tools_per_request", 24)

        all_defs = self.get_tool_definitions()
        if len(all_defs) <= max_tools:
            return all_defs

        text = (user_message or "").lower()
        words = {w for w in re.findall(r"[a-z_]{3,}", text)}

        def score(td: dict[str, Any]) -> float:
        ...
        ranked = sorted(all_defs, key=score, reverse=True)
    ...
    async def execute_tool(
        ...
        async for _event, data in self.execute_tool_stream(tool_call, context):
            # execute_tool_stream yields already-serialized result dicts.
            final_result = (
                data if isinstance(data, ToolResult) else ToolResult.from_dict(data)
            )
