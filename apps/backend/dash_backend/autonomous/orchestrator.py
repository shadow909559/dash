...
class Orchestrator:
    ...
    async def _plan(self, task: str, max_steps: int) -> list[OrchestratorStep]:
        # Try distilled planner first (instant, <10ms)
        from dash_backend.autonomous.distilled_planner import plan_with_fallback
        distilled = plan_with_fallback(task, max_steps)

        if distilled.matched_pattern and distilled.steps:
            logger.info(
                "Distilled plan hit '%s' in %.1fms (%d steps)",
                distilled.matched_pattern, distilled.planning_time_ms, len(distilled.steps),
            )
            return [
                OrchestratorStep(
                    id=str(uuid.uuid4())[:8], index=i,
                    description=s.description,
                    step_type=StepType(s.step_type),
                    agent=s.agent,
                )
                for i, s in enumerate(distilled.steps)
            ]
