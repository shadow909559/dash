...
// Applied via session API to all responses. Prevents XSS, code injection,
// and unauthorized external resource loading.
...
let mainWindow = null;
let orbWindow = null; // Dedicated orb window
...
ipcMain.handle("auth:device-token", async () => {
    try {
        ...
        if (!parsed.device_token)
            return { ok: false, reason: "identity file missing device_token" };
        cachedDeviceToken = parsed.device_token;
        return { ok: true, token: cachedDeviceToken };
    }
    catch (err) {
        console.error("[auth] failed to read DASH identity file:", err.message);
...
ipcMain.handle("window:set-mode", async (_, mode) => {
...
ipcMain.handle("startup:set-settings", async (_, settings) => {
...
ipcMain.handle("startup:get-settings", () => {
    ...
    const launchArgs = login.launchArguments || "";
    return {
        ...
        startMinimized: startupPrefs.startMinimized ||
            login.openAsHidden ||
...
function applyCSP() {
    ...
    console.log("[Security] CSP and security headers applied");
}
// ── Window creation ─────────────────────────────────────────────────────────
