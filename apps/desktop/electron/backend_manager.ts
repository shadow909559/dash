...
interface HealthProbe {
  ...
  bodySnippet: string;
  errorType: "none" | "connection_refused" | "timeout" | "http_error" | "invalid_response";
  errorMessage: string;
  latencyMs?: number;
}
...
export class BackendManager {
  ...
  private usePythonDirect = false;
  private ownedPid: number | null = null;
  private trackingPid: number | null = null;
  ...
  constructor() {
    ...
    if (app.isPackaged) {
      ...
      if (fs.existsSync(bundledExe)) {
        ...
        this.usePythonDirect = true;
        console.warn(
          `[BackendManager] DashBackend.exe and venv not bundled — falling back to system Python for the backend (autostart will fail with a clear error if Python is absent)`,
        );
      }
    } else {
  ...
  private async waitForBackend(maxAttempts = 120, interval = 500): Promise<void> {
  ...
  private probeDashHealth(): Promise<HealthProbe> {
    return (async () => {
  ...
  private httpHealth(healthPath: string): Promise<HealthProbe> {
    return new Promise((resolve) => {
      ...
      const req = http.get(
        ...
        (res) => {
          ...
          res.on("end", () => {
            ...
            let errorType: "none" | "connection_refused" | "timeout" | "http_error" | "invalid_response" = "none";
            let errorMessage = "";
      ...
      req.on("error", (err) => {
        const errorType: "connection_refused" | "timeout" | "http_error" | "invalid_response" = 
          (err as any).code === "ECONNREFUSED" ? "connection_refused" : "http_error";
        resolve({ 
          ok: false, 
          isDash: false, 
          statusCode: null, 
  ...
  private bodyLooksLikeDash(body: string): boolean {
    ...
    try {
      const json = JSON.parse(body) as { status?: string; service?: string; app?: string };
  ...
  private getListeningPid(): Promise<number | null> {
    return new Promise((resolve) => {
  ...
  private getProcessCommandLine(pid: number): Promise<string> {
    return new Promise((resolve) => {
      const ps = spawn(
        "powershell.exe",
        [
  ...
  private commandLineLooksLikeDash(cmd: string): boolean {
    const lower = cmd.toLowerCase();
    return (
      lower.includes("dash_backend") ||
  ...
  private scheduleRecover(reason: string): void {
    if (this.stopping || this.recoverScheduled) return;
  ...
  private startHealthCheck(): void {
    ...
    this.healthTimer = setInterval(() => {
      ...
      this.enqueue(async () => {
        if (this.stopping) return;
        if (this.lifecycle === "starting" || this.lifecycle === "recovering") return;
        ...
        if (looksLikeDash || (this.ownedPid != null && pid === this.ownedPid)) {
          ...
          if (this.unhealthyTicks >= STALE_REPLACE_THRESHOLD) {
            ...
            this.log(
              "stale DASH backend detected — replacing with one healthy instance",
              { pid, cmd: cmd.slice(0, 180), consecutiveFailures: this.unhealthyTicks },
            );
  ...
  private stopPid(pid: number | null, allowed: boolean): Promise<void> {
    if (!pid || !allowed) return Promise.resolve();
    return new Promise((resolve) => {
  ...
  private clearOwnedIf(pid: number | null): void {
  ...
  private writeLock(pid: number | null, owned: boolean): void {
    try {
      fs.writeFileSync(
        this.lockPath,
        JSON.stringify({
  ...
  private clearLockIfOwned(): void {
    try {
      if (fs.existsSync(this.lockPath)) {
        const raw = fs.readFileSync(this.lockPath, "utf8");
        const data = JSON.parse(raw) as { electronPid?: number; owned?: boolean };
