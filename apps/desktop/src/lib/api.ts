...
export const commandCenter = {
  upcoming: (days = 7) =>
    request<{ window_days: number; count: number; items: UpcomingItem[] }>(
      `/executive/goals/upcoming?days=${days}`
    ),
  ...
  suggestions: (limit = 10) =>
    request<{ count: number; suggestions: ProactiveSuggestion[] }>(
      `/proactive/suggestions?limit=${limit}`
    ),

  briefing: () => request<BriefingData>("/proactive/briefing"),

  ackSuggestion: (signalId: string, title?: string) =>
    request<{ ok: boolean; signal_id: string }>("/proactive/ack", {
      method: "POST",
      body: { signal_id: signalId, title },
    }),
