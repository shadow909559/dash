...
function resolveBadge(
  item: NavItem,
  counts: { deadlines: number; suggestions: number }
): { value: string | number; color: string } | null {
  if (item.id === "home") {
    const total = counts.deadlines + counts.suggestions;
    if (total === 0) return null;
    const color =
      counts.deadlines > 0
        ? "var(--dash-warning)"
        : "var(--dash-accent-secondary)";
    return { value: total, color };
  }
  return null;
}
      ...
      {items.map((item) => {
        ...
        return (
          <NavLink
            key={item.id}
            to={item.path}
            title={!isExpanded ? `${item.label}${badge ? ` (${badge.value})` : ""}` : undefined}
            style={({ isActive }) => ({
              display: "flex",
              alignItems: "center",
              gap: 12,
                ...
                {!isExpanded && badge && (
                  <span
                    style={{
                      ...
                      border: "1.5px solid var(--dash-bg)",
                      zIndex: 1,
                    }}
                    aria-label={`${badge.value} items needing attention`}
                  />
                )}
